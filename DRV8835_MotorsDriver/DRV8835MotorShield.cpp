...
void DRV8835MotorShield::initPinsAndMaybeTimer()
{
  ...
  sigmaDeltaSetup(0, 10000);//channel, frequency
  sigmaDeltaAttachPin(25, 1);//pin, channel

  //SigmaDelta Setup(pin 16, channel 0)
  sigmaDeltaSetup(0, 10000);//channel, frequency
  sigmaDeltaAttachPin(26, 2);//pin, channel
...
void DRV8835MotorShield::setM1Speed(int speed)
{
  ...
  OCR1A = speed;
#else
  sigmaDeltaWrite(1, speed * 51 / 80); // default to using analogWrite, mapping 400 to 255
#endif 
...
void DRV8835MotorShield::setM2Speed(int speed)
{
  ...
  OCR1B = speed;
#else
  sigmaDeltaWrite(2, speed * 51 / 80); // default to using analogWrite, mapping 400 to 255
#endif
